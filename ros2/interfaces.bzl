    ...
    *,
    ...
    for input in inputs:
        for template in output_templates:
            ...
            # XXX: If the extension is .idl, then we assume it's a message file.
            #      This is required as the type description generator runs heuristics on the IDL
            #      AST to determine the action/service/message-ness, and replicating that logic here
...
def _idl_adapter_aspect_impl(target, ctx):
    ...
    outputs = []
    # Source files that are already .idl files, and don't need to be processed.
    existing_idl_files = []
    ...
    for src in srcs:
        ...
        # Don't need to re-process IDL files.
        if extension == "idl":
            # XXX: If we are passed an IDL file directly we assume it is a message file.
            #      This is required as the type description generator runs heuristics on the IDL
            #      AST to determine the action/service/message-ness, and replicating that logic here
            #      would require a lot of effort.
            idl_tuples.append("{}:msg/{}.idl".format(real_output_dir, stem))
            idl_output = ctx.actions.declare_file("{}/msg/{}.idl".format(output_dir, stem))
            existing_idl_files.append(idl_output)
            ctx.actions.symlink(output=idl_output, target_file=src)
        else:
    ...
    return [
        IdlAdapterInfo(
            direct_files = outputs + existing_idl_files,
            files = depset(
                direct = outputs + existing_idl_files,
...
ros2_interface_library_rule = rule(
    ...
    attrs = {
        "srcs": attr.label_list(
            allow_files = [".action", ".msg", ".srv", ".idl"],
            mandatory = True,
        ),
        "deps": attr.label_list(
            providers = [RosInterfaceInfo],
